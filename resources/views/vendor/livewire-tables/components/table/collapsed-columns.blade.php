...
@if ($this->collapsingColumnsAreEnabled() && $this->hasCollapsedColumns())
    ...
    <tr x-data
        ...
        {{ $attributes->merge($customAttributes)->class([
                'hidden bg-white dark:bg-gray-700 dark:text-white rappasoft-striped-row' =>
                    $isTailwind && ($customAttributes['default'] ?? true) && $rowIndex % 2 === 0,
            ])->class([
                'hidden bg-gray-50 dark:bg-gray-800 dark:text-white rappasoft-striped-row' =>
                    $isTailwind && ($customAttributes['default'] ?? true) && $rowIndex % 2 !== 0,
            ])->class([
                'd-none bg-light rappasoft-striped-row' =>
                    $isBootstrap && $rowIndex % 2 === 0 && ($customAttributes['default'] ?? true),
            ])->class([
                'd-none bg-white rappasoft-striped-row' =>
                    $isBootstrap && $rowIndex % 2 !== 0 && ($customAttributes['default'] ?? true),
                ...
                @foreach ($columns as $colIndex => $column)
                    ...
                    <p wire:key="{{ $tableName }}-row-{{ $row->{$primaryKey} }}-collapsed-contents-{{ $colIndex }}"
                        @class([
                            ...
                            'block mb-2 md:hidden' =>
                                $isTailwind &&
                                !$column->shouldCollapseAlways() &&
                                !$column->shouldCollapseOnTablet() &&
                                $column->shouldCollapseOnMobile(),
                            'block mb-2 lg:hidden' =>
                                $isTailwind &&
                                !$column->shouldCollapseAlways() &&
                                ($column->shouldCollapseOnTablet() ||
                                    $column->shouldCollapseOnMobile()),
